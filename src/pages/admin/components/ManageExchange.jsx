...
import { useGetExchangeQuery, useDeleteExchangeMutation } from "../../../redux/api/exchangeApi";
...
const ManageExchangeContent = () => {
  ...
  const handleDelete = async (record) => {
    ...
    confirm({
      ...
      content: `Are you sure you want to delete ${getItemName(record.items_id)}?`,
      okText: 'Delete',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        try {
          const response = await deleteExchange(record.id); // Menggunakan fungsi deleteExchange dari useDeleteExchangeMutation
          if (response.error) {
  ...
  return (
    <div className="">
      ...
      <Spin spinning={exchangeLoading}>
        <Table columns={columns} dataSource={exchangeData ? exchangeData.data.map(item => ({ ...item, key: item.id })) : []} />
      </Spin>
    </div>
  );
};
